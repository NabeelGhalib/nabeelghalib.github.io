...
- **Data Preparation**:  
- One-Hot Encoding: Encoded categorical variables such as Gender and Time Spent to make them suitable for machine learning algorithms.
...
- Train-Test Split: Divided the dataset into training (80%) and testing (20%) sets.
    ...
    df_encoded = pd.get_dummies(df, columns=['Gender', 'Time_Spent'], drop_first=True)
    ...
    X = df_encoded.drop('Need_Checkup', axis=1)
    y = df_encoded['Need_Checkup']
